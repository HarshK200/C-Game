...
// =================================================================================
//                              TEMP TILEMAP GENERATION
// =================================================================================

TileChunk* TEMP_ChunkCreateAndInit(AppMemory* memory)
{
...
void TEMP_ChunkQueueRender(AppMemory* memory, TileChunk* chunk, RenderData* render_data)
{
    ...
    for (int tile_y = 0; tile_y < CHUNK_SIZE; tile_y++)
    {
        for (int tile_x = 0; tile_x < CHUNK_SIZE; tile_x++)
        {
            ...
            Tile tile = GetTileInChunk(chunk, tile_coords);
            tile_coords.x = tile_x;
            tile_coords.y = tile_y;
            tile_pos = TileToWorldCoords(chunk->ChunkCoords, tile_coords);
            render_command.Transforms[tile_idx] = ModelMat4(tile_pos, tile_scale);
            tile_sprite.TexelCoords.x = 0;
            tile_sprite.TexelCoords.y = 16 * tile.TileType;
